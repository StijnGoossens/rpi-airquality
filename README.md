...
```
This will start the monitoring script and Streamlit dashboard on startup. Logs (including the optional keep-alive ping) will be printed to the specified files under your home folder. The last two lines are watchdogs, both checking every 5 minutes: `scripts/dashboard_watchdog.sh` restarts Streamlit if it crashed (e.g. the `StreamClosedError` issue above), and `scripts/wifi_watchdog.sh` reconnects `wlan0` via `nmcli` if it dropped its IP (observed after a router hiccup or Pi reboot) — losing Wi-Fi also cuts off SSH access and the outdoor-weather API calls in `monitor.py`.
